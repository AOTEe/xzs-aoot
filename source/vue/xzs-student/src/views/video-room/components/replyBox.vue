<template>
  <div class="reply-box">
    <div class="box-normal">
      <div class="reply-box-avatar" nftavatartype="0">
        <div class="bili-avatar" style="width: 48px;height:48px;">
          <img class="bili-avatar-img bili-avatar-face bili-avatar-img-radius"
               data-src="//i0.hdslb.com/bfs/face/6ae1e37a930fc04e1e5ced2cb6d82a731f73734b.jpg@160w_160h_1c_1s_!web-avatar-comment.avif"
               alt=""
               src="//i0.hdslb.com/bfs/face/6ae1e37a930fc04e1e5ced2cb6d82a731f73734b.jpg@160w_160h_1c_1s_!web-avatar-comment.avif">
          <span class="bili-avatar-icon bili-avatar-right-icon  bili-avatar-size-48"></span>
        </div>
      </div>
      <div class="reply-box-warp">
        <textarea v-model="contentText" class="reply-box-textarea" :placeholder="placeholderText"></textarea>
      </div>
      <div class="reply-box-send" @click="sendText()">
        <div class="send-text">发布</div>
      </div>
    </div>
  </div>
</template>

<script>
import CommonApi from "@/api/commonRequest";
export default {
  name: "ReplyBox",
  data(){
    return{
      contentText : "",
    }
  },
  props:{
    topicId: null,
    topicType : null,
    placeholderText : "",
    previousCommentId : "",
    index : null
  },
  methods :{
    sendText(){
      var param = new FormData()
      if (this.index != null) {
        param.append("parent", this.previousCommentId)
        param.append("content", this.placeholderText + this.contentText)
      } else {
        param.append("content", this.contentText)
      }
      param.append("topicId", this.topicId)
      param.append("topicType", this.topicType)
      param.append("userId", this.$store.state.user.userInfo.id)
      console.log(param)
      CommonApi.formDataPostApi('/api/student/comment/add',param ).then(res => {

        alert("评论成功！")
        //分情况 需要将评论数据push到reply或subReply列表中
        //还需要置空相应的属性
        debugger
        this.contentText = ""
        if (this.index != null) {
          //父子组件间通信
          this.$parent.subReplyPlaceholder = ""
          this.$parent.subPreviousCommentId = ""
          this.contentText = ""
          this.$parent.commentList[this.index].subReply.push(res.data)
        }else {
          //兄弟组件间通信
          this.$bus.$emit('appendReplyList',res.data)
        }
      })
    }
  }
}
</script>

<style scoped>
.reply-box .box-normal {
  display: flex;
  height: 50px;
  transition: .2s;
}
.reply-box .box-normal .reply-box-avatar {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 80px;
  height: 50px;
}
.bili-avatar {
  display: block;
  position: relative;
  background-image: url(data:image/gif;base64,);
  -webkit-background-size: cover;
  background-size: cover;
  border-radius: 50%;
  margin: 0;
  padding: 0;
}
.bili-avatar-img-radius {
  border-radius: 50%;
}
.bili-avatar-img {
  border: none;
  display: block;
  -o-object-fit: cover;
  object-fit: cover;
}
.reply-box .box-normal .reply-box-send .send-text {
  position: absolute;
  z-index: 1;
  font-size: 16px;
  color: #ffffff;
}
.reply-box .box-normal .reply-box-send:after {
  content: "";
  position: absolute;
  opacity: .5;
  width: 100%;
  height: 100%;
  border-radius: 4px;
  background-color: #00aeec;
}
.reply-box .box-normal .reply-box-send {
  display: flex;
  justify-content: center;
  align-items: center;
  position: relative;
  flex-basis: 70px;
  margin-left: 10px;
  border-radius: 4px;
  cursor: pointer;
}
.reply-box .box-normal .reply-box-warp {
  position: relative;
  flex: 1;
}
.reply-box .box-normal .reply-box-warp .reply-box-textarea {
  width: 100%;
  height: 100%;
  padding: 5px 10px;
  border: 1px solid #f1f2f3;
  border-radius: 6px;
  background-color: #f1f2f3;
  font-family: inherit;
  line-height: 38px;
  color: #18191c;
  resize: none;
  outline: none;
}
.reply-box .box-normal .reply-box-warp {
  position: relative;
  flex: 1;
}
</style>
