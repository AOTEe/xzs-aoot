...
public class TaskExamServiceImpl extends BaseServiceImpl<TaskExam> implements TaskExamService {
    ...
    public void edit(TaskRequestVM model, User user) {
        ...
        if (actionEnum == ActionEnum.ADD) {
            ...
            //清空试卷任务的试卷Id，后面会统一设置
            List<String> paperIds = JsonUtil.toJsonListObject(textContent.getContent(), TaskItemObject.class)
                    .stream()
        ...
        //更新试卷的taskId
        List<String> paperIds = model.getPaperItems().stream().map(d -> d.getId()).collect(Collectors.toList());
        examPaperMapper.updateTaskPaper(taskExam.getId(), paperIds);
        model.setId(taskExam.getId());
    }

    @Override
    public TaskRequestVM taskExamToVM(String id) {
